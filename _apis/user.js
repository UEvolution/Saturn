...
router.use('/view', async (req, res) => {
  return await limitBody(req.body, [['id', 'nice_name']])
    ...
    .then(r => r.dataValues)
    .then(() => resJson ? res.sendSuccess({data: resJson}) : Promise.reject({msg: '暂无数据!'}))
    .catch(error => res.sendError(error))
})
...
router.use('/info', async (req, res) => {
  return await users.findOne({where: {id: req.user.id}})
    .then(data => res.sendSuccess({msg: '获取成功', data}))
    .catch(error => res.sendError(error))
})
